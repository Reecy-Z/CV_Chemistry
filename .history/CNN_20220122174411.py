...
def generate_dataloader(category,category_dcit):
    train_fea = []
    train_label = []
    for i in range(split):
        print(i)
        ...
        mol = data[category][i]
        mol_img = Image.open('./' + category + '\\' + category_dcit[mol] + '.png')
    ...
    data_loader = torch.utils.data.DataLoader(feaTrain, batch_size = batch_size, shuffle=True)
...
for epoch in range(epochs):
    model.train()
    for batch_idx,(cat_fea,label) in Catalyst_loader:
        cat_fea,label = cat_fea.to(DEVICE),label.to(DEVICE)
        output = model(cat_fea)
