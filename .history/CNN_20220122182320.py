...
def generate_dataloader(category,category_dict):
    train_fea = []
    train_label = []
    for i in range(split):
        print(i)
    ...
    # data_loader = torch.utils.data.TensorDataset(feaTrain)
...
for epoch in range(epochs):
    ...
    for batch_idx,(cat_fea,label) in enumerate(Catalyst_loader):
        ...
        imine_fea = list(Imine_loader)[batch_idx].to(DEVICE)
        thiol_fea = list(Thiol_loader)[batch_idx].to(DEVICE)

        output_cat = model(cat_fea)
        output_imine = model(imine_fea)
        output_thiol = model(thiol_fea)


        print(output_cat.size())
        print(output_imine.size())
        print(output_thiol.size())
