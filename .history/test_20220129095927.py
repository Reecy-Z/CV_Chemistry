...
src_output = layer_norm(output + residual)
...
residual = src_output
output = nn.ReLU()(conv1(src_output.transpose(1, 2)))
output = conv2(output).transpose(1, 2)
src_output = layer_norm(output + residual)

tgt_word_emb = nn.Embedding(tgt_vocab_size,d_model)
tgt_pos_emb = nn.Embedding.from_pretrained(get_sinusoid_encoding_table(tgt_len,d_model),freeze=True)
tgt_output = tgt_word_emb(output) + tgt_pos_emb(torch.LongTensor([[5,1,2,3,4]]))
