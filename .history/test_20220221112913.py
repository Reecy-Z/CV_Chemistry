...
scores.masked_fill_(attn_mask, -1e9) # Fills elements of self tensor with value where mask is one.True的位置用value填充
attn = nn.Softmax(dim=-1)(scores)
...
conv1 = nn.Conv1d(in_channels=d_model, out_channels=d_ff, kernel_size=2)
...
dec_enc_attn_mask = get_attn_pad_mask(output, input)
