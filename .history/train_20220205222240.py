...
train_data = utils.generate_Traindata_loader(file,split,dicts,batch_size,Dir)
test_data = utils.generate_Testdata_loader(file,split,end,dicts,batch_size,Dir)
...
for epoch in range(epochs):
    ...
    r2_train = []
    model.train()
    for batch_idx,(fea,label) in enumerate(train_data):
        fea,label = torch.from_numpy(np.array(fea)),torch.from_numpy(np.array(label))
    ...
    r2_test = []
    with torch.no_grad():
        for batch_idx, (fea, label) in enumerate(test_data):
            fea,label = torch.from_numpy(np.array(fea)),torch.from_numpy(np.array(label))
            fea, label = fea.to(DEVICE), label.to(DEVICE)
            output = model(fea.to(torch.float))
            pred = output
