        ...
        if len(train_label_batch) == batch_size:
            ...
            if (len(train_fea) == num) and ((split%batch_size) != 0):
                if len(train_fea_batch) == split - len(train_fea)*batch_size:
                    ...
                    continue        elif len(train_label) == num:
        if len(train_label)*batch_size != split:
            train_label.append(np.array(train_label_batch))
            break
        else:
            break
    ...
    for i in range(split,end):
        ...
        for index,category in enumerate(columns):
            ...
            if len(test_fea_batch) == columns_len * batch_size:
                test_fea_batch = []
            elif len(test_fea) == num:
                test_fea.append(test_fea_batch)
                break

        test_label_batch.append(data['Output'][i])
        if len(test_label_batch) == batch_size:
            test_label.append(test_label_batch)
            test_label_batch = []
        elif len(test_label) == num:
            test_label.append(test_label_batch)
            break
