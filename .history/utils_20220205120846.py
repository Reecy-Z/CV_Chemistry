    ...
    for i in range(split):
        ...
        for index,category in enumerate(columns):
            ...
            if len(train_fea_batch) == columns_len * batch_size:
                ...
                if (len(train_fea) == num) and ((split%batch_size) != 0):
        ...
        if len(train_label_batch) == batch_size:
            ...
            if (len(train_label) == num) and ((split%batch_size) != 0):
...
def generate_Testdata_loader(file,split,end,dicts,batch_size,Dir):
    ...
    for i in range(split,end):
        ...
        for index,category in enumerate(columns):
            ...
            if len(test_fea_batch) == columns_len * batch_size:
                test_fea.append(np.array(test_fea_batch))
                test_fea_batch = []
            else:
                if (len(test_fea) == num) and (((end-split)%batch_size) != 0):
                    if len(test_fea_batch) == split - len(test_fea)*batch_size:
                        test_fea.append(np.array(test_fea_batch))
                        break
                    else:
                        continue
        ...
        if len(test_label_batch) == batch_size:
            ...
            if (len(test_label) == num) and ((split%batch_size) != 0):
                if len(test_label_batch) == split - len(test_label)*batch_size:
                    test_label.append(np.array(test_label_batch))
                    break
                else:
                    continue
