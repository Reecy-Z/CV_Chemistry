    ...
    for i in range(split):
        ...
        for index,category in enumerate(columns):
            ...
            if len(train_fea_batch) == columns_len * batch_size:
                ...
                if len(train_fea_batch) == (split - len(train_fea)*batch_size)*columns_len:
                    train_fea.append(np.array(train_fea_batch))
                    break
                else:
                    continue
        ...
        if len(train_label_batch) == batch_size:
            ...
            if len(train_label_batch) == (split - len(train_label)*batch_size)*columns_len:
                train_label.append(np.array(train_label_batch))
                break
            else:
                continue
    ...
    for i in range(split,end):
        ...
        for index,category in enumerate(columns):
            ...
            if len(test_fea_batch) == columns_len * batch_size:
                ...
                if len(test_fea_batch) == ((end-split) - len(test_fea)*batch_size)*columns_len:
                    test_fea.append(np.array(test_fea_batch))
                    break
                else:
                    continue
        ...
        if len(test_label_batch) == batch_size:
            ...
            if len(test_label_batch) == ((end-split) - len(test_label)*batch_size)*columns_len:
                test_label.append(np.array(test_label_batch))
                break
            else:
                continue
