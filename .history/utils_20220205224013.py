...
def generate_Testdata_loader(file,split,end,dicts,batch_size,Dir):
    ...
    for i in range(split,end):
        ...
        test_reaction_label.append(np.array(data['Output'][i]))
        test_data_batch[0].append(test_reaction_fea)
        test_data_batch[1].append(test_reaction_label)

        if len(test_data_batch[0]) == batch_size:
            test_data.append(np.array(test_data_batch))
            test_data_batch = ([],[])
        elif (len(test_data) == num) and (((end-split)%batch_size) != 0):
            if len(test_data_batch[0]) == (end-split) - len(test_data)*batch_size:
